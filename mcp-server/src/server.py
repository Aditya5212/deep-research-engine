...
# ---------------------------------------------------------------------------
# Logger — global instance shared by all tools
# ---------------------------------------------------------------------------
logging.basicConfig(
...
logger = logging.getLogger("mcp_server.tavily")
...
def tavily_search(
    ...
    try:
        ...
        logger.info(
            "[tavily_search] query=%r | depth=%s | topic=%s | max_results=%d",
            query, search_depth, topic, max_results,
        )
        result = client.search(**params)
        logger.info(
            "[tavily_search] done | results=%d | response_time=%s",
        ...
        logger.error("[tavily_search] failed | query=%r | %s: %s", query, type(e).__name__, e)
...
def tavily_extract(
    ...
    try:
        ...
        logger.info(
            "[tavily_extract] urls=%d | depth=%s | format=%s",
            len(urls), extract_depth, output_format,
        )
        result = client.extract(**params)
        logger.info(
            "[tavily_extract] done | success=%d | failed=%d | response_time=%s",
            len(result.get("results", [])),
            len(result.get("failed_results", [])),
            result.get("response_time", "?"),
        )
        return result

    except Exception as e:
        logger.error("[tavily_extract] failed | urls=%s | %s: %s", urls, type(e).__name__, e)
...
def tavily_crawl(
    ...
    try:
        ...
        logger.info(
            "[tavily_crawl] url=%r | depth=%d | breadth=%d | limit=%d",
            url, max_depth, max_breadth, limit,
        )
        result = client.crawl(**params)
        logger.info(
            "[tavily_crawl] done | results=%d | response_time=%s",
            len(result.get("results", [])),
            result.get("response_time", "?"),
        )
        return result
        ...
        logger.error("[tavily_crawl] failed | url=%r | %s: %s", url, type(e).__name__, e)
...
def tavily_map(
    ...
    try:
        ...
        logger.info(
            "[tavily_map] url=%r | depth=%d | breadth=%d | limit=%d",
            url, max_depth, max_breadth, limit,
        )
        result = client.map(**params)
        logger.info(
            "[tavily_map] done | urls_found=%d | response_time=%s",
            len(result.get("results", [])),
            result.get("response_time", "?"),
        )
        return result
        ...
        logger.error("[tavily_map] failed | url=%r | %s: %s", url, type(e).__name__, e)
...
def tavily_research(
    ...
    try:
        ...
        logger.info(
            "[tavily_research] input=%r | model=%s | stream=%s",
            input[:120] if len(input) > 120 else input, model, stream,
        )
        result = client.research(**params)
        logger.info(
            "[tavily_research] done | status=%s | response_time=%s",
            result.get("status", "?"),
            result.get("response_time", "?"),
        )
        return result
        ...
        logger.error("[tavily_research] failed | model=%s | %s: %s", model, type(e).__name__, e)
